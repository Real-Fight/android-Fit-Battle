...
class PoseMatcher {
    ...
    fun match(pose: Pose, targetPose: TargetPose): Pair<Boolean, Float> {
        ...
        targetPose.targets.forEach { target ->
            ...
            angle = calculateAngle(firstLandmark!!, middleLandmark!!, lastLandmark!!)
            val targetAngle = target.angle
            Log.d(Constant.TAG, "${firstLandmark.landmarkType} $angle !!!!!!!!!!!!!!!")
            if (!anglesMatch(angle, targetAngle)) {
                return Pair(false, -1F)
            }
        }
        return Pair(true, angle.toFloat())
    }
